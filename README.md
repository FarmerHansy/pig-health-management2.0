...
智牧医典（生猪健康管理智慧医药系统）聚焦生猪健康与用药管理，构建面向养殖场景的检索增强生成（RAG）智能诊疗系统，覆盖 AI 兽医问答、疾病管理、药品管理、文章资讯等核心模块。系统以 **SpringBoot3 + RAG + Ollama + Spring AI + DeepSeek + Mysql8.0 + Mybatis-Plus** 为技术底座，通过知识库检索为回答提供辅助依据，帮助养殖户或兽医更快定位病情并获得可执行的治疗建议。

## 提交检查说明/变更说明

- 已清理后端与前端源码中的临时调试输出（如 `System.out.println`、`console.log`）。
- 仅做最小化改动，不涉及业务逻辑与接口行为调整。
