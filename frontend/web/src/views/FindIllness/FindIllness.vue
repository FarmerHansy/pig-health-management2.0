<template>
  <div class="find-illness-root">
    ...
    <div class="container">
      <!-- 侧边栏分类 -->
      <aside class="sidebar">
        <div class="sidebar-title">疾病分类</div>
        <ul class="type-list">
      ...
      <main class="content-area">
        ...
        <div class="top-controls">
          <el-input
            ...
            prefix-icon="Search"
          />
          <div class="stats">{{ filteredItems.length }} 条结果</div>
        </div>
        ...
        <div class="illness-list">
          ...
          >
            <div class="card-header">
              <h3 class="illness-title">{{ item.illnessName }}</h3>
              <div class="card-meta">
                <span class="category">{{ item.kindName || getCategoryNameById(item.kindId) }}</span>
                <span class="views">
            ...
            <div class="card-content">
              <div class="info-row">
                ...
                <p class="info-text">{{ stripText(item.illnessSymptom) }}</p>
              </div>

              <div class="info-row">
                <span class="info-label">原因</span>
                <p class="info-text">{{ stripText(item.includeReason) }}</p>
              </div>
            </div>

            <div class="card-footer">
              <span class="date">{{ formatDate(item.createTime) }}</span>
              <el-button type="primary" size="small" @click.stop="gotoDetail(item)">
                详情
              </el-button>
            </div>
          </div>
